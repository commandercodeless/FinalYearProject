...
public class GameUI extends javax.swing.JFrame implements Client.IObserver{
    ...
    private void doTurn(Action thisTurn){
        int colour;
        if (thisTurn.playerId == white.player.userId){
            colour = 0;
        } else{
            colour = 1;
        }
        addPiece(new Piece(thisTurn.startLocationX, thisTurn.startLocationY,colour));
        TilePanel newPanel = tiles.get((thisTurn.startLocationX * 8) + thisTurn.startLocationY);
        try {
            BufferedImage image;
            String teamString;
            if (board.tiles[thisTurn.startLocationX][thisTurn.startLocationY].piece.team == 0){
                teamString = "White";
            ...
            image = ImageIO.read(new File("reversi\\" + teamString + ".png"));
            ...
            newPanel.revalidate();
            newPanel.repaint();
            for (int i = 0; i < thisTurn.flippedX.size(); i++){
                board.tiles[thisTurn.startLocationX][thisTurn.startLocationY].piece.team = colour;
                newPanel = tiles.get((thisTurn.flippedX.get(i) * 8) + thisTurn.flippedY.get(i));
                if (newPanel.picLabel != null){
                    newPanel.remove(newPanel.picLabel);
                }
                newPanel.add(picLabel);
        ...
        if (board.validMoves(colour)){ } else{
            int victor = board.getVictor();
            if (victor == 0){
                victory(white.player.userId);
            } 
            if (victor == 1){
                victory(black.player.userId);
            }
            if (victor == -1){
                victory(-1);
            }
    ...
    private void makePieces(){
        addPiece(new Piece(3,3,0));
        addPiece(new Piece(4,4,0));
        addPiece(new Piece(4,3,1));
        addPiece(new Piece(3,4,1));
    }
        ...
        TilePanel(int i, int j){
            ...
            if (board.tiles[i][j].piece != null){
                try {
                    ...
                    BufferedImage image = ImageIO.read(new File("reversi\\" + teamString + ".png"));
                ...
                public void mouseClicked(MouseEvent e) {
                    if (e.getButton() == 1){
                        if (board.tiles[x][y].piece == null){
                            if(board.checkValid(x, y, turn)){
                                Piece newPiece = new Piece(x,y,turn);
                                board.tiles[x][y].piece = newPiece;
                                List<Piece> changed = board.turnOther(x,y,turn);
                                for (int i = 0; i < changed.size(); i++){
                                    changed.get(i).team = turn;
                                    TilePanel newPanel = tiles.get((changed.get(i).x * 8) + changed.get(i).y);
                                    if (newPanel.picLabel != null){
                                        newPanel.remove(newPanel.picLabel);
                                    }
                                    newPanel.add(picLabel);
                                ...
                                Action thisTurn = new Action();
                                thisTurn.playerId = clientData.userId;
                                thisTurn.startLocationX = startx;
                                thisTurn.startLocationY = starty;
                                for (int i = 0; i < changed.size(); i++){
                                    thisTurn.flippedX.add(changed.get(i).x);
                                    thisTurn.flippedY.add(changed.get(i).y);
                                }
                                ...
                                try {
                                    BufferedImage image;
                                    String teamString;
                                    if (board.tiles[x][y].piece.team == 0){
                                        teamString = "White";
                                    } else{
                                        teamString = "Black";
                                    }
