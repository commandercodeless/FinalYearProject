...
public class GameUI extends javax.swing.JFrame implements Client.IObserver{
    ...
    private void doTurn(Action thisTurn){
        ...
        if (Math.abs(thisTurn.startLocationX - thisTurn.endLocationX) != 1){
            clearTilesBetween(thisTurn.startLocationX, thisTurn.startLocationY, thisTurn.endLocationX, thisTurn.endLocationY);
        }
        ...
        try {
            ...
            if (board.tiles[thisTurn.endLocationX][thisTurn.endLocationY].piece.team == 0){
                ...
                if (thisTurn.endLocationX == 7){
                    board.tiles[thisTurn.endLocationX][thisTurn.endLocationY].piece.name = "king";
                }
            } else{
                teamString = "Black";
                turn = 0;
                ...
                if (thisTurn.endLocationX == 0){
                    board.tiles[thisTurn.endLocationX][thisTurn.endLocationY].piece.name = "king";
                }
            }
            if (board.tiles[thisTurn.endLocationX][thisTurn.endLocationY].piece.name.equals("king")){
                teamString += "King";
            }
            image = ImageIO.read(new File("draughts\\" + teamString + ".png"));
        ...
        if (board.victory()){
            victory(thisTurn.playerId);
        }
    }
    
    private void clearTilesBetween(int startx, int starty, int endx, int endy){
        if (startx - endx == starty - endy){
            if (startx > endx){
                for (int i = 1; i < startx - endx; i++){
                    board.tiles[startx - i][starty - i].piece = null;
                    TilePanel thisPanel = tiles.get(((startx - i) * 8) + (starty - i));
                    thisPanel.remove(thisPanel.picLabel);
                    thisPanel.picLabel = null;
                }
            } else{
    ...
    private void makePieces(){
        addPiece(new Piece(ids,0,1,0));
        addPiece(new Piece(ids,0,3,0));
        addPiece(new Piece(ids,0,5,0));
        addPiece(new Piece(ids,0,7,0));
        addPiece(new Piece(ids,1,0,0));
        addPiece(new Piece(ids,1,2,0));
        addPiece(new Piece(ids,1,4,0));
        addPiece(new Piece(ids,1,6,0));
        addPiece(new Piece(ids,2,1,0));
        addPiece(new Piece(ids,2,3,0));
        addPiece(new Piece(ids,2,5,0));
        addPiece(new Piece(ids,2,7,0));
        
        addPiece(new Piece(ids,7,0,1));
        addPiece(new Piece(ids,7,2,1));
        addPiece(new Piece(ids,7,4,1));
        addPiece(new Piece(ids,7,6,1));
        addPiece(new Piece(ids,6,1,1));
        addPiece(new Piece(ids,6,3,1));
        addPiece(new Piece(ids,6,5,1));
        addPiece(new Piece(ids,6,7,1));
        addPiece(new Piece(ids,5,0,1));
        addPiece(new Piece(ids,5,2,1));
        addPiece(new Piece(ids,5,4,1));
        addPiece(new Piece(ids,5,6,1));
    }
        ...
        TilePanel(int i, int j){
            ...
            if (board.tiles[i][j].piece != null){
                try {
                    ...
                    BufferedImage image = ImageIO.read(new File("draughts\\" + teamString + ".png"));
                        ...
                        if (e.getButton() == 2){
                            ...
                            if (e.getButton() == 1){
                                if (startx == -1){
                                    ...
                                    endx = x;
                                    endy = y;
                                    if (board.checkValid(startx, starty, endx, endy ,board.tiles[startx][starty].piece.team)){
                                        ...
                                        if (isHost){
                                            try {
                                                //send to other players                                                 <-DON'T FORGET TO DO THIS
                                                for (int i = 0; i < spectators.size(); i++){
